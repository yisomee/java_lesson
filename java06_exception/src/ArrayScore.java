	...
	public static void main(String[] args) {
		...
		do {
			try {
				...
				for(int i=0;i<cnt;i++) {// 행
					...
					for(int j=0;j<title.length; j++) {//과목: j 과목명 에 추가하라. // 열
						try {	
							...
							//점수의 범위가 0~100사이가 아니면 예외를 발생시킨다. MyException 예외를 발생시킴
							if(jumsu[i][j]<=0 || jumsu[i][j]>100) {
							//if(!(jumsu[i][j]>=0 && jumsu[i][j]<100)) {}
							
								//강제 예외 발생 throw는 예외를 강제로 발생시킨다.
								throw new MyException(title[j]+"점수는 0~100사이의 값이어햐 합니다.");
							}
							...
							System.out.println("점수는 0~100사이의 값이어야 합니다.");
							j--;
						}
					}
				}
				...
				System.out.println("학생수 또는 점수는 0보다 큰 정수이어야 합니다.");
			}
		}while(true);
	}
}
